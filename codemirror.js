...
var CodeMirror = (function() {
  ...
  function CodeMirror(place, givenOptions) {
    ...
    function handleTab(shift) {
      ...
      case "spin":
          var n;
           var section = getStateBefore(sel.from.line + 1).section;
           var tabs = mode.tabs[section];
        if (posEq(sel.from, sel.to)) {
            if (shift) {
                if (sel.from.ch) {
                    var text = lines[sel.from.line].text;
                    var backTo = text.indexOf(' ', tabs.indexOf(tabs.charAt(sel.from.ch - 1)));
                    if (backTo >= 0) {
                        var start = sel.from.ch;
                        while (start > backTo && text.charAt(start - 1) == ' ') start--;
                        replaceRange('', {line: sel.from.line, ch: start}, sel.from);
                    }
                }
            } else {
                n = tabs.charCodeAt(sel.from.ch) - sel.from.ch;
                if (isNaN(n)) n = 4;
              while (n) {
        ...
        var minIndent = 1000;
        var toline = sel.to.line;
        if (sel.to.ch == 0) toline--
        for (var i = sel.from.line; i <= toline; ++i) {
            var ind = lines[i].indentation();
            if (ind < minIndent && lines[i].text.match(/\S/)) minIndent = ind;
        }
        if (minIndent === 1000) break;
        if (shift) {
            if (minIndent) {
                var backTo = tabs.indexOf(tabs.charAt(minIndent - 1));
            } else {
                break;
            }
            if (options.indentUnit = minIndent - backTo) {
                for (var i = sel.from.line; i <= toline; ++i) indentLine(i, "subtract");
            }
            break;
        } else {
            var fwdTo = tabs.charCodeAt(minIndent);
            if (isNaN(fwdTo)) fwdTo = minIndent + 4;
            options.indentUnit = fwdTo - minIndent;
               for (var i = sel.from.line; i <= toline; ++i) indentLine(i, "add");
               break;            
        }
      }
      return true;
